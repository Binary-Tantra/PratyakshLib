...
public class CycleSelector : UIBase, IPointerInteractable
{
    ...
    {
        get => options;
        set => options = value ?? Array.Empty<string>();
    }
    ...
    public CycleSelector(string[] options, int selectedIndex, int posX, int posY, int width, int height, Action<CycleSelector>? onSelectionChanged, object? payload = null, int fontSize = 15, Drawable? parent = null) : base(parent)
    {
